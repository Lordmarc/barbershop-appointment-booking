    ...
    <div className="relative rounded-2xl overflow-hidden flex flex-col bg-[#1a1f14] border border-white/[0.07] shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
      ...
      <div className="relative h-52 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 bg-[#14180e]/80 border border-[#c8a84b]/40 rounded-full px-2.5 py-1">
          <span className="text-[11px] font-bold tracking-widest text-[#c8a84b]">PHP {service.price}</span>
        </div>
      </div>

      {/* Info */}
      <div className="p-4 flex items-center justify-between">
        <p className="text-[#f0ede6] font-bold text-base">{service.name}</p>

        {/* Actions */}
        <div className="flex gap-2">
          <button
            onClick={() => onEdit(service)}
            className="w-8 h-8 rounded-lg bg-white/[0.06] border border-white/10 flex items-center justify-center text-gray-400 hover:bg-white/[0.12] transition-all cursor-pointer"
          >
            <FiEdit2 size={13} />
          </button>
          <button
